...
image_response = requests.get(
    "https://jigsawstack.com/preview/object-detection-example-input.jpg"
)
...
const result = await jigsaw.vision.object_detection({
...
<CodeGroup>
```javascript object_detection_direct.js 
import { JigsawStack } from "jigsawstack";
...
const response = await jigsaw.vision.object_detection(blob, {
  features: ["object_detection"],
  annotated_image: true,
  return_type: "url",
});
...
```python object_detection_direct.py
from jigsawstack import JigsawStack
import requests
...
response = jigsaw.vision.object_detection(
    imageblob,
    {"features": ["object_detection"], "annotated_image": True, "return_type": "url"},
)
...
curl -X POST \
  ...
  -F "features=[\"object_detection\"]" \
  -F "annotated_image=true" \
  -F "return_type=url" \
  https://api.jigsawstack.com/v1/object_detection
```
</CodeGroup>
