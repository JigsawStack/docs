...
<ParamField body="element_prompts" type="array<string>">
  Items on the page to scrape (maximum 5). E.g. "Plan price", "Plan title". Either `element_prompts` or `selectors` is required, or both.
</ParamField>

<ParamField body="selectors" type="array<string>">
  CSS selectors to scrape (maximum 5). E.g. "li > a", "div > .points". Either `element_prompts` or `selectors` is required, or both.
</ParamField>
